    ...
    */
    ...
    while(!stop) {
        ...
        for(j = 1; j < J-1; j++) {
            ...
            for(i = 1; i < I-1; i++) {
                F[i-1] = (dt/2.)*w[i][j] + (1.-2.*sig)*psi[i][j] + sig*(psi[i][j+1] + psi[i][j-1]);
            }
        ...
        for(i = 1; i < I-1; i++) {
            // Filling source term F:
            for(j = 1; j < J-1; j++) {
                F[j-1] = (dt/2.)*w[i][j] + (1.-2.*sig)*psihat[i][j] + sig*(psihat[i+1][j] + psihat[i-1][j]);
            }
            F[0] = F[0] + sig*psi[i][0];
            F[J-3] = F[J-3] + sig*psi[i][J-1];
        ...
        for(i = 0; i < I; i++) {      /// ESSE LOOP TEM QUE SER POR ULTIMO PRA QUE O CONTORNO SUPERIOR SEJA PREENCHIDO.
            w[i][0] = -2.*psi[i][1]/pow(h, 2);             // Inferior
            what[i][0] = -2.*psi[i][1]/pow(h, 2.);             // Inferior
            w[i][J-1] = -2.*psi[i][J-2]/pow(h, 2.) - 2.0/h;   // Superior
            what[i][J-1] = -2.*psi[i][J-2]/pow(h, 2.) - 2.0/h;   // Superior
        }
        ...
        for(j = 1; j < J-1; j++) {
            for(i = 1; i < I-1; i++) {
                RhoX[i-1] = Re*(psi[i+1][j] - psi[i-1][j])/4.;
                RhoY[i-1] = Re*(psi[i][j+1] - psi[i][j-1])/4.;
                
                Di[i-1] = -sig*(RhoY[i-1] + 1.);
                Dp[i-1] = 1. + 2.*sig;
                Ds[i-1] = sig*(RhoY[i-1] - 1.);
                
                F[i-1] = (1.-2.*sig)*w[i][j] + sig*(RhoX[i-1] + 1)*w[i][j+1] - sig*(RhoX[i-1] - 1)*w[i][j-1];
            }
        ...
        for(i = 1; i < I-1; i++) {
            for(j = 1; j < J-1; j++) {
                RhoX[j-1] = Re*(psi[i+1][j] - psi[i-1][j])/4.;
                RhoY[j-1] = Re*(psi[i][j+1] - psi[i][j-1])/4.;
            ...
            F[0] = F[0] - sig*(RhoX[0] - 1.)*w[i][0];
            F[J-3] = F[J-3] + sig*(RhoX[J-3] + 1.)*w[i][J-1];
        ...
        if(R <= 1.0e-6) {
            stop = 1;
            cx = snprintf(buffer, 100,  "./Results/Navier-Stokes/omega_%d_%0.0f_dt%d_%d.csv", Nel, Re, which_h, n);
            print_results(w, I, J, buffer); //"./Results/omega_64_1000_h2s10.csv");
            cx = snprintf(buffer, 100, "./Results/Navier-Stokes/psi_%d_%0.0f_dt%d_%d.csv", Nel, Re, which_h, n);
