...
void boussinesq() {
    ...
    // For Temperature:
    double F1[J] = {0.};
    double Di1[J] = {0.}, Dp1[J] = {0.}, Ds1[J] = {0.};   // Diagonals from A matrix
    double *U1 = NULL;
    ...
    while(!stop) {
        ...
        for(j = 0; j < J; j++) {
            for(i = 1; i < I-1; i++) {
                Di[i-1] = -sig*(RhoY[i][j] + 1.);
                Dp[i-1] = (1./alpha_T + 2.*sig);
                Ds[i-1] = sig*(RhoY[i][j] - 1.);

                if(j == 0) {
                    F[i-1] = (1./alpha_T - 2.*sig)*T[i][j] + sig*(RhoX[i][j] + 1.)*T[i][j+1] + sig*(1. - RhoX[i][j])*T[i][j+1];
                } else if(j == J-1) {
                    F[i-1] = (1./alpha_T - 2.*sig)*T[i][j] + sig*(RhoX[i][j] + 1.)*T[i][j-1] + sig*(1. - RhoX[i][j])*T[i][j-1];
                } else {
                    F[i-1] = (1./alpha_T - 2.*sig)*T[i][j] + sig*(RhoX[i][j] + 1.)*T[i][j+1] + sig*(1. - RhoX[i][j])*T[i][j-1];
            ...
            F[0] = F[0] + sig*(RhoY[1][j] + 1.)*That[0][j];
            F[I-3] = F[I-3] - sig*(RhoY[I-2][j] - 1.)*That[I-1][j];
        ...
        for(i = 1; i < I-1; i++) {
            for(j = 0; j < J; j++) {
                Di1[j] = sig*(RhoX[i][j] - 1.);
                Dp1[j] = (1./alpha_T + 2.*sig);
                Ds1[j] = -sig*(RhoX[i][j] + 1.);

                F1[j] = (1./alpha_T - 2.*sig)*That[i][j] + sig*(1. - RhoY[i][j])*That[i+1][j] + sig*(1. + RhoY[i][j])*That[i-1][j];
            }

            Ds1[0] = Ds1[0] + sig*(RhoX[i][0] - 1.);
            Di1[J-1] = Di1[J-1] - sig*(RhoX[i][J-1] + 1.);

            U1 = myTridiag(Di1, Dp1, Ds1, F1, J);
            for(j = 0; j < J; j++) {
                T[i][j] = U1[j];
            }
            free(U1);
        ...
        for(j = 1; j < J-1; j++) {
            for(i = 1; i < I-1; i++) {
                ...
                F[i-1] = theta[i-1][j-1] + (1./alpha_w - 2.*Pr*sig)*w[i][j] + sig*(RhoX[i][j] + Pr)*w[i][j+1] + sig*(Pr - RhoX[i][j])*w[i][j-1];
            }
        ...
        for(i = 1; i < I-1; i++) { 
            for(j = 1; j < J-1; j++) {
                ...
                F[j-1] = theta[i-1][j-1] + (1./alpha_w - 2.*Pr*sig)*what[i][j] + sig*(Pr - RhoY[i][j])*what[i+1][j] + sig*(Pr + RhoY[i][j])*what[i-1][j];
            }
        ...
        if(R <= 1.0e-6) {
            stop = 1;
            cx = snprintf(buffer, 100,  "./Results/Natural Convection Boussinesq/omega_%d_%0.2f_%0.0f_%0.2f_%0.2f_%0.2f_dt%d_%d.csv", Nel, Pr, Ra, alpha_psi, alpha_w, alpha_T, which_h, n);
            print_results(w, I, J, buffer);
            cx = snprintf(buffer, 100, "./Results/Natural Convection Boussinesq/psi_%d_%0.2f_%0.0f_%0.2f_%0.2f_%0.2f_dt%d_%d.csv", Nel, Pr, Ra, alpha_psi, alpha_w, alpha_T, which_h, n);
            print_results(psi, I, J, buffer);
            cx = snprintf(buffer, 100, "./Results/Natural Convection Boussinesq/T_%d_%0.2f_%0.0f_%0.2f_%0.2f_%0.2f_dt%d_%d.csv", Nel, Pr, Ra, alpha_psi, alpha_w, alpha_T, which_h, n);
